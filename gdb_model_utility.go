...
// getModel 如果<safe>为true，则创建并返回当前模型的克隆模型，否则返回当前模型。
...
// mappingAndFilterToTableFields 将给定的字段名映射并更改为真正的表字段名。
...
// filterDataForInsertOrUpdate 不使用插入/更新操作的数据筛选功能。注意，对于“omit empty”特性，它不过滤列表项，列表项也是映射类型。
